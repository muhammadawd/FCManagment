<template>
  ...
  <card class="card" :title="$ml.get('add_course')">
    <div>
      <form @submit.prevent>
        <div class="row">
          <div class="col-md-3">
  ...
  export default {
    ...
    nameWithLang ({ name, parentName }) {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.ALL_SEC_DEPTS)
            .then((response) => {
              ...
              if (response.status) {
                vm.all_sections = [''].concat(response.data.result);
                return null;
              }
              vm.all_sections = [];
      ...
      addCourse() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.ADD_COURSE, request_data)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
